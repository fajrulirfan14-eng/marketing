...
async function loadHomeDataCacheAside(uid, role) {
  // ── SEMUA ROLE (kurir, hunter, sales): full online, tanpa cache IDB ──

  // 1. users/{uid} — langsung dari Firestore
  let userData = {};
  try {
...
window.initHomeView = async function(){
    ...
    roleContent.innerHTML = `
      <div class="home-sales-wrapper" id="homeSalesWrapper">
        <div class="home-sales-top">
          ...
          <div style="display:flex;gap:8px;align-items:center;">
            <button class="home-sales-plus secondary" id="homeSalesInputBtn" onclick="window.showView('inputharian')">
              <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round">
  ...
  if(avatar){
    ...
    if(user.fotoURL){
      ...
      avatar.innerHTML = `
        <img src="${user.fotoURL}" alt="${user.nama}" class="home-avatar-img">
      `;
